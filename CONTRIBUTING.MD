...
For example, the [`color` feature](https://github.com/devcontainers/feature-starter#color) provides an enum of three possible options (`red`, `gold`, `green`).  If no option is provided in a user's `devcontainer.json`, the value is set to "red".
...
For each feature added there should be a corresponding tests created under `test/<feature>/`.

To make testing and running easier, see [all make commands](makefile).
